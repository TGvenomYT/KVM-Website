...
function getSpan(size, isFeatured) {
  ...
  if (isFeatured) {
    if (s === "tall") return "row-span-3 md:col-span-2 md:row-span-3";
    if (s === "huge") return "row-span-3 md:col-span-3 md:row-span-3";
...
export async function getCourses() {
  const [courses, courseFaculty, timetable, faculty] = await Promise.all([
    fetchTab("Courses"),
  ...
  return courses.map((c) => {
    ...
    const courseTimetable = timetable.filter((t) => t.Course === c.Title);

    const batchOrder = [];
    const batchMap = new Map();
    for (const t of courseTimetable) {
      const batchName = (t.Batch || "").trim() || "Regular Batch";
      if (!batchMap.has(batchName)) {
        batchMap.set(batchName, []);
        batchOrder.push(batchName);
      }
      batchMap.get(batchName).push(t);
    }

    const batches = batchOrder.map((name) => {
      const rows = batchMap.get(name);
      const days = DAY_ORDER.map((day) => ({
        day,
        slots: rows
          .filter((t) => t.Day === day)
          .map((t) => ({ time: t.Time, subject: t.Subject })),
      })).filter((d) => d.slots.length > 0);
      return { name, timetable: days };
    });
